...
- **ITokenPayload** (from common/user.interface.ts)
  - **userId**: string (immutable \_id — source of truth for auth)
  - **username**: string (convenience field, may be stale after rename)
  - **iat?**: number
  - **exp?**: number
...
| :-- | :-- | :-- | :-- | :-- |
| **GET** | /users | Get all usernames (Admin only) | ISuccess (string[] payload) | _None_ |
| **GET** | /users/:username | Get user account details | ISuccess (IUserAccount payload) | _None_ |
...
### **3.1 Get All Usernames (GET /account/users)**

- **Path Parameters:** _None_
- **Headers:** Authorization: Bearer token (JWT)
- **Authorization Rules:** Administrators only. Used to populate the username dropdown in the Admin Manage Accounts view.
- **Response:** ISuccess with name 'UsersRetrieved' and payload of string[] (all usernames).

### **3.2 Get User Account (GET /account/users/:username)**
...
### **3.3 Update Account Status (PATCH /account/users/:username/status)**
...
### **3.4 Update Privilege Level (PATCH /account/users/:username/privilege)**

- **Body Fields:** privilegeLevel (IPrivilegeLevel, required)
- **Authorization:** Only Administrators. Members are strictly forbidden.

### **3.5 Update Username (PATCH /account/users/:username/username)**
...
### **3.6 Update Email (PATCH /account/users/:username/email)**
...
### **3.7 Update Password (PATCH /account/users/:username/password)**
...
- **UsersRetrieved:** Returns string[] of all usernames (Admin only).
...
| **LastAdministrator** | Cannot inactivate or demote the sole active Administrator. |
| **UserNotFound** | Target user does not exist. |
| **InvalidPassword** | Does not meet complexity rules. |
| **WeakPassword** | Password fails strength validation (length, chars). |
| **UsernameExists** | New username is already taken. |
| **InvalidUsername** | Username too short or on reserved list. |
| **InvalidEmail** | Email not a valid CMU email address. |
| **MissingUsername** | Username parameter not provided. |
| **MissingPassword** | Password field not provided. |
| **MissingEmail** | Email field not provided. |
| **MissingToken** | JWT token not provided in Authorization header. |
| **InvalidToken** | JWT token is invalid or expired. |
...
UsersRetrieved, AccountRetrieved, StatusUpdated, PrivilegeUpdated, UsernameUpdated, EmailUpdated, PasswordUpdated.
